<?php
class conexionDGRRFEM{
	
	function conexionDGRRFEM()
	{
		$this->host = 'localhost';
		$this->db = 'dgr2';
		$this->user = 'root';
	...
	function select($sql,$bitacora = false)
	{
		...
		*/
		...
		$resultado = mysql_query($sql) or die("<p style='background:white; color: red; padding:5px'>".mysql_error()." <br> - <b>Consulta Select</b>: ".nl2br($sql)."</p>");
		return $resultado;
	}
	
	function insert($sql,$bitacora = true)
	{
		$resultado = mysql_query($sql) or die(mysql_error()." <br> - <b>Consulta Insert</b>: ".nl2br($sql));
		return $resultado;
	}
	
	function update($sql,$bitacora = true)
	{
		$resultado = mysql_query($sql) or die(mysql_error()." <br> - <b>Consulta Update</b>: ".nl2br($sql));
		return $resultado;
	}
	
	function delete($sql,$bitacora = true)
	{
		$resultado = mysql_query($sql) or die(mysql_error()." <br> - <b>Consulta Delete</b>: ".nl2br($sql)); 
		return $resultado;
	}
}
?>